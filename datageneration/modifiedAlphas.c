...
#define AGE_UNDER_35 0
#define AGE_OVER_45 1

#define WITH_PASSENGERS 0
#define WITHOUT_PASSENGERS 1
...
typedef struct ID_data
{
	...
	int context_good[2][2][2][2][2][2];
	int context_bad[2][2][2][2][2][2];
	...
	double bias_context[2][2][2][2][2][2];
}ID_data;
...
int behavior_simulation(int weather, int visibility, int rush_hour, int gender, int age, int passenger, ID_data *target_ID);
double behavior_probability(int weather, int visibility, int rush_hour, int gender, int age, int passenger, ID_data *target_ID);
...
double estimate_temp_trust(ID_data *ID_data_base, double* similarity_list, int target_ID, int weather, int visibility, int rush_hour, int gender, int age, int passenger, int num_ID);
double estimate_initial_trust(ID_data *ID_data_base, double* similarity_list, int target_ID, int weather, int visibility, int rush_hour, int gender, int age, int passenger, int num_ID, int *sorted_sim_list);
				...
				for (l = 0; l < 2; l++)
				{
                    ...
                    {   
                        for (n = 0; n < 2; n++)
			            {
                            for (o = 0; o < 2; o++)
			                {
					            new_IDs[i].context_good[j][k][l][m][n][o] = 0;
					            new_IDs[i].context_bad[j][k][l][m][n][o] = 0;
                            }
                        }
                    }
				}
				...
				for (l = 0; l < 2; l++)
				{
                    ...
                    {   
                        for (n = 0; n < 2; n++)
			            {
                            for (o = 0; o < 2; o++)
			                {
					            new_IDs[i].bias_context[j][k][l][m][n][o] = ((rand() % (BIAS_RANGE + 1)) - (BIAS_RANGE / 2)) / (double)1000;
                            }
                        }
                    }
				}
				...
				for (l = 0; l < 2; l++)
				{
                    ...
                    {   
                        for (n = 0; n < 2; n++)
			            {
                            for (o = 0; o < 2; o++)
			                {
                                temp_ptr[i].context_good[j][k][l][m][n][o] = base_ID_data_base[i].context_good[j][k][l][m][n][o];
                                temp_ptr[i].context_bad[j][k][l][m][n][o] = base_ID_data_base[i].context_bad[j][k][l][m][n][o];
                            }
                        }
                    }
				}
				...
				for (l = 0; l < 2; l++)
				{
                    ...
                    {   
                        for (n = 0; n < 2; n++)
			            {
                            for (o = 0; o < 2; o++)
			                {
					            temp_ptr[i].bias_context[j][k][l][m][n][o] = ((rand() % (BIAS_RANGE + 1)) - (BIAS_RANGE / 2)) / (double)1000;
                            }
                        }
                    }
				}
...
{
	int i, j, r, is_in;
	int id, weather, visibility, rush_hour, gender, age, passenger;
	...
	for (i = 0; i < num_data; i++)
	{
        ...
        r = rand() % 1000;
        if(r < 755) gender = MALE;
        else gender = FEMALE;

        r = rand() % 1000;
        if(r < 802) age = AGE_UNDER_35;
        else age = AGE_OVER_45;
        
        r = rand() % 1000;
        if(r < 411) passenger = WITH_PASSENGERS;
        else passenger = WITHOUT_PASSENGERS;
        ...
        new_data_base[i].gender = gender;
        new_data_base[i].age = age;
        new_data_base[i].passenger = passenger;
		...
		temp_probability = behavior_probability(weather, visibility, rush_hour, gender, age, passenger, &((*ID_data_base)[id]));
		...
		for (j = 0; j < NUMBER_BEHAVIOR; j++)
		{
			...
			{
				(*ID_data_base)[id].bad++;
				(*ID_data_base)[id].context_bad[weather][visibility][rush_hour][gender][age][passenger]++;
				new_data_base[i].bad++;
			}
			else
			{
				(*ID_data_base)[id].good++;
				(*ID_data_base)[id].context_good[weather][visibility][rush_hour][gender][age][passenger]++;
...
int behavior_simulation(int weather, int visibility, int rush_hour, int gender, int age, int passenger, ID_data *target_ID)
{
    ...
    //     probability_base->weather[weather] * probability_base->visibility[visibility] * probability_base->rush_hour[rush_hour] * probability_base->gender[gender] * probability_base->age[age] * probability_base->passenger[passenger]
    //     + target_ID->bias_context[weather][visibility][rush_hour] - ((rand() % (PERSONAL_RANGE + 1)) / (double)1000);
	// }

	malicious_probability = behavior_probability(weather, visibility, rush_hour, gender, age, passenger, target_ID) * 1000;
...
double* I_similartiy_estimation(ID_data *ID_data_base, int ID_trustee, int num_ID)
{
	...
	double tv_trustee[2][2][2][2][2][2];
	double tv_reference[MAX_NUM_ID][2][2][2][2][2][2];
			...
			for (k = 0; k < 2; k++)
			{
                for (m = 0; m < 2; m++)
                {
                    for (n = 0; n < 2; n++)
                    {
                        for (o = 0; o < 2; o++)
                        {
                            if (ID_data_base[ID_trustee].context_good[i][j][k][m][n][o] == 0 && ID_data_base[ID_trustee].context_bad[i][j][k][m][n][o] == 0) tv_trustee[i][j][k][m][n][o] = NO_HISTORY;
                            else tv_trustee[i][j][k][m][n][o] = ID_data_base[ID_trustee].context_good[i][j][k][m][n][o] / (double)(ID_data_base[ID_trustee].context_good[i][j][k][m][n][o] + ID_data_base[ID_trustee].context_bad[i][j][k][m][n][o]);
                        }
                    }
                }
			}
				...
				for (k = 0; k < 2; k++)
				{
                    for (m = 0; m < 2; m++)
                    {
                        for (n = 0; n < 2; n++)
                        {
                            for (o = 0; o < 2; o++)
                            {
                                if (ID_data_base[l].context_good[i][j][k][m][n][o] == 0 && ID_data_base[l].context_bad[i][j][k][m][n][o] == 0) tv_reference[l][i][j][k][m][n][o] = NO_HISTORY;
                                else tv_reference[l][i][j][k][m][n][o] = ID_data_base[l].context_good[i][j][k][m][n][o] / (double)(ID_data_base[l].context_good[i][j][k][m][n][o] + ID_data_base[l].context_bad[i][j][k][m][n][o]);
                            }
                        }
                    }
				}
				...
				for (l = 0; l < 2; l++)
				{
                    for (m = 0; m < 2; m++)
                    {
                        for (n = 0; n < 2; n++)
                        {
                            for (o = 0; o < 2; o++)
                            {
                                if (tv_trustee[j][k][l][m][n][o] >= 0 && tv_reference[i][j][k][l][m][n][o] >= 0)
                                {
                                    diff_sum = diff_sum + fabs(tv_trustee[j][k][l][m][n][o] - tv_reference[i][j][k][l][m][n][o]);
...
double estimate_temp_trust(ID_data *ID_data_base, double* similarity_list, int target_ID, int weather, int visibility, int rush_hour, int gender, int age, int passenger, int num_ID)
{
	...
	for (i = 0; i < num_ID; i++)
	{
		...
		if (ID_data_base[i].context_good[weather][visibility][rush_hour][gender][age][passenger] > 0 && ID_data_base[i].context_bad[weather][visibility][rush_hour][gender][age][passenger] > 0)
		{
			trust_value = trust_value + (similarity_list[i] * ID_data_base[i].context_good[weather][visibility][rush_hour][gender][age][passenger] / (double)(ID_data_base[i].context_good[weather][visibility][rush_hour][gender][age][passenger] + ID_data_base[i].context_bad[weather][visibility][rush_hour][gender][age][passenger]));
			leveraging_count++;
		}
	}
...
double estimate_initial_trust(ID_data *ID_data_base, double* similarity_list, int target_ID, int weather, int visibility, int rush_hour, int gender, int age, int passenger, int num_ID, int *sorted_sim_list)
{
	...
	for (i = 0; i < num_ID; i++)
	{
		...
		if (ID_data_base[temp_id].context_good[weather][visibility][rush_hour][gender][age][passenger] > 0 && ID_data_base[temp_id].context_bad[weather][visibility][rush_hour][gender][age][passenger] > 0)
		{
			trust_value = trust_value + (similarity_list[temp_id] * ID_data_base[temp_id].context_good[weather][visibility][rush_hour][gender][age][passenger] / (double)(ID_data_base[temp_id].context_good[weather][visibility][rush_hour][gender][age][passenger] + ID_data_base[temp_id].context_bad[weather][visibility][rush_hour][gender][age][passenger]));
			leveraging_count++;
		}
	}
...
void trust_simulation(int num_ID, int num_controller_ID_data, int num_neighbor_ID_data, int num_data, double weight, double threshold, int prob_forge, double mal_vehicle_portion, FILE *fp, double *I_trust_result, double *ob_trust_result, double *none_result)
{
	...
	int weather, visibility, rush_hour, gender, age, passenger;
	...
	for (i = 0; i < num_ID; i++)
	{
		...
		{
			if(controller_ID_data_base[i].context_good[weather][visibility][rush_hour][gender][age][passenger] != 0 || controller_ID_data_base[i].context_bad[weather][visibility][rush_hour][gender][age][passenger] != 0)
			{
				temp_target_tv = controller_ID_data_base[i].context_good[weather][visibility][rush_hour][gender][age][passenger] / (double)(controller_ID_data_base[i].context_good[weather][visibility][rush_hour][gender][age][passenger] + controller_ID_data_base[i].context_bad[weather][visibility][rush_hour][gender][age][passenger]);
	...
	for (i = 0; i < num_unknown; i++)
	{
		...
		temp_tv = estimate_temp_trust(controller_ID_data_base, similarity_list[i], target_ID, weather, visibility, rush_hour, gender, age, passenger, num_ID);
	...
	for (i = 0; i < NUMBER_INTERACTION_VEHICLE; i++)
	{
		...
		list_I_sharing_trust_value[i] = estimate_initial_trust(controller_ID_data_base, interaction_sim_list[i], target_ID, weather, visibility, rush_hour, gender, age, passenger, num_ID, sorted_sim_list);
	...
	for (j = 0; j < NUMBER_INTERACTION_VEHICLE; j++)
	{
		...
		list_trust_results[j] = 1 - behavior_probability(weather, visibility, rush_hour, gender, age, passenger, &(controller_ID_data_base[target_ID]));
	}
	...
	for (i = 0; i < NUMBER_INTERACTOIN_BEHAVIOR; i++)
	{
		...
		{
			target_ID = list_interaction[j];
			list_behavior_results[j] = behavior_simulation(weather, visibility, rush_hour, gender, age, passenger, &(controller_ID_data_base[target_ID]));
		}
		...
		for (j = 0; j < NUMBER_INTERACTION_VEHICLE; j++)
		{
			...
			fprintf(fp, "%i \t %i \t %i \t %i \t %i \t %i \t %i \t %i \t %i \t %i \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %i \t %i \n",
                ...
                probability_base->gender[MALE],
				probability_base->gender[FEMALE],
				probability_base->age[AGE_UNDER_35],
				probability_base->age[AGE_OVER_45],
				probability_base->passengers[WITH_PASSENGERS],
				probability_base->passengers[WITHOUT_PASSENGERS],
				I_sharing_decision,
