...
data = pd.read_csv('../sampledata/local_data100_ce_db_hm.csv', header=0)
...
class trustEnv:
    ...
    def __init__(self, thrvalue, deltavalue, bdelta, rewvalue, beta):
        ...
        self.originals = [thrvalue, deltavalue, bdelta, rewvalue]
        ...
        self.average_beta = defaultdict(list)
    ...
    def connect(self):
        ...
        self.accrewcollection = self.db['new_tv_data_wbeta']
    ...
    def step2(self, action, car_id): 
        ...
        self.cumulative_beta += self.beta
    ...
    def save_avg_accuracy(self, run_counts, name): #! iterate and make average of the iterations.
        ...
        for j in range(len(self.epoch_accuracy[0])):
            ...
            final_beta.append(temp[4]/run_counts)
            
        print("Accuracy: {}".format(final_acc[-1]))
        print("beta: {}".format(final_beta[-1]))

        # print(final_rew[-1])
        row = {"id": str(name), "v_d": name.v_d, "v_lr": name.v_lr, "v_df": name.v_df, "v_eps": name.v_eps, "v_fd": name.v_fd, "v_s": name.v_s, "v_i": name.v_i, "yvalue": final_acc, "avg_dtt": final_dtt, "avg_gt": final_gt, "cum_rew": final_rew, "avg_beta": final_beta}
