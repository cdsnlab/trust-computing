...
def reconstruct_rtm(i, s,mvp, mbp, oap, interval):
    allcombinations=[]
    for output in named_product(v_i = i, v_s=s, v_mvp=mvp, v_mbp=mbp, v_oap=oap, v_interval=interval):
...
rwcombo = reconstruct_rtm(i, s, mvp, mbp, oap, interval)
...
for j in rwcombo:
    ...
    x = np.arange(int(xvalue)/mydoc_dtm[0]['v_interval'])
