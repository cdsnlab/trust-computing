...
def evaluate(threshold, samplelist, data, intnumb):
    ...
    for i in (samplelist):
        ...
        if tv_id > threshold:
            ...
            decision[i]=1
        # print(good_history, bad_history, tv_id,  int(data['status'][i]==1))
...
# for output in named_product(v_i=[90],v_s = [59999], v_mvp=[0.2], v_mbp=[0.5], v_oap=[0.2]):
    ...
    for i in range (run_counts):
        ...
        while True:    
            ...
            if threshold == 5:
                hitmin=True
                hitmax=False
            # print(interaction_number, threshold, PPV, NPV, accuracy[i][-1])
